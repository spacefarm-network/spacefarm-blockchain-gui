...
import { Card, FormatLargeNumber, Link, Loading, TooltipIcon, Flex } from '@spacefarm/core';
import {
  unix_to_short_date,
  hex_to_array,
  arr_to_hex,
...
export default function Block() {
  ...
  const poolReward = mojo_to_spacefarm(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_spacefarm(calculateBaseFarmerReward(blockRecord.height));

  const spacefarmFees = blockRecord.fees
    ? mojo_to_spacefarm(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: spacefarmFees ? `${spacefarmFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    >
      <Card
        title={(
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Spacefarm
              blockchain
            </Trans>
          </BlockTitle>
        )}
