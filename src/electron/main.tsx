...
import spacefarmEnvironment from '../util/spacefarmEnvironment';
import spacefarmConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!spacefarmEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    spacefarmConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!spacefarmConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !spacefarmConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (spacefarmConfig.manageDaemonLifetime()) {
        spacefarmEnvironment.startSpacefarmDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Spacefarm Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/spacefarm-network/spacefarm-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/spacefarm-network/spacefarm-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/spacefarm-network/spacefarm-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/spacefarm-network/spacefarm-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/spacefarm-network/spacefarm-blockchain/issues',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal('https://twitter.com/spacefarm_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Spacefarm Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Spacefarm' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Spacefarm Blockchain' }),
            click: () => {
              openAbout();
            },
          },
