...
const PY_FOLDER = '../spacefarm/daemon';
...
const getSpacefarmVersion = () => {
  let version = null;
  const exePath = getExecutablePath('spacefarm');
  // first see if we can get a spacefarm exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or spacefarm is on the path
    try {
      version = child_process
